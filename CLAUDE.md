...
- `src/App.tsx` wires `QueryClientProvider` → `TooltipProvider` → both toasters (`ui/toaster` and `ui/sonner`) → `BrowserRouter`. Only two routes exist: `/` → `pages/Index`, catch-all `*` → `pages/NotFound`. Add new routes *above* the catch-all.
- `src/pages/Index.tsx` is the whole marketing page — it composes the section components from `src/components/` in a fixed vertical order (Navbar → Hero → Storytelling → Why → LogoMarquee → PublishedPhoto → WhatWeDo → HowWeWork → Promise → Join → PromisePhotos → Contact → Footer, plus a floating `WhatsAppFloat`). Section order lives here; reordering the page means editing this file.
